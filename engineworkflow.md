...
**Sprint 0 is the most important.** Everything downstream lives or dies on the content schema. Getting that JSON contract right before touching engine code means security-adventure authors and engine developers can work in parallel from Sprint 1 onward without stepping on each other.

**The engine/content split is strict by design.** smrt's `engine/` folder never contains story content; security-adventure never contains renderer code. The GitHub Action is the seam between them — this keeps your game portable and makes it possible to re-skin or swap content without touching the engine.
...
smrt is a game engine for security-adventure.
